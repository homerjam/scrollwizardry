(function (global, factory) {
  ...
  (global = global || self, factory(global.ScrollWizardry = {}));
  ...
  var TPL = {
    ...
    trigger: function trigger(color) {
      ...
      wrapper.appendChild(inner); // element
  ...
  function () {
    function Indicator(scene, options) {
      ...
      this._boundsContainer = options.parent && Util.elements(options.parent)[0]; // prepare bounds elements

      this._elemStart.firstChild.textContent += " ".concat(options.name);
      this._elemEnd.textContent += " ".concat(options.name);
      ...
      this._vertical = null;
      this._ctrl = null; // set public variables
    ...
    _createClass(Indicator, [{
      ...
      value: function add() {
        ...
        if (!isDocument && Util.css(this._boundsContainer).position === 'static') {
          ...
          Util.css(this._boundsContainer, {
            position: 'relative'
          });
        ...
        this.scene.on('shift.plugin_addIndicators', this._handleBoundsParamsChange.bind(this)); // updates trigger & bounds (will add elements if needed)
      ...
      value: function _addBounds() {
        var v = this._ctrl.info('vertical'); // apply stuff we didn't know before...
      ...
      value: function _addTriggerGroup() {
        ...
        this.triggerGroup = newGroup; // update right away
      ...
      value: function _updateTriggerGroup() {
        ...
        var closeEnough = 0.0001; // Have a group, check if it still matches
          ...
          if (Math.abs(group.triggerHook - triggerHook) < closeEnough) {
            ...
            if (this.triggerGroup) {
              ...
              if (this.triggerGroup.members.length === 1) {
                ...
                this._ctrl.updateTriggerGroupPositions(this.triggerGroup); // Log.log(0, "trigger", options.name, "->", "removing from previous member list");
  ...
  var SCENE_STATE_AFTER = 'AFTER'; // list of options that trigger a `shift` event
  ...
  function () {
    function Scene(options) {
      ...
      this._cssClassElems = [];
      this._tween = null;
      this._indicator = null; // add getters/setters for all possible options
      ...
      this.validate = {
        duration: function duration(val) {
          ...
          if (!lodash.isNumber(val) || val < 0) {
            if (this._durationUpdateMethod) {
              this._durationUpdateMethod = null;
              throw Error("Invalid return value of supplied function for option \"duration\": ".concat(val));
            } else {
              throw Error("Invalid value for option \"duration\": ".concat(val));
            }
          }
        ...
        offset: function offset(val) {
          ...
          if (!lodash.isNumber(val)) {
            throw Error("Invalid value for option \"offset\": ".concat(val));
          }
        ...
        triggerElement: function triggerElement(val) {
          ...
          if (val) {
            ...
            if (el !== undefined && el.parentNode) {
              ...
              throw Error("Element defined in option \"triggerElement\" was not found: ".concat(val));
            }
          }
        ...
        triggerHook: function triggerHook(val) {
          ...
          if (lodash.isNumber(val)) {
            ...
            throw Error("Invalid value for option \"triggerHook\": ".concat(val));
          }
        ...
        loglevel: function loglevel(val) {
          ...
          if (!lodash.isNumber(val) || val < 0 || val > 3) {
            throw Error("Invalid value for option \"loglevel\": ".concat(val));
          }
    ...
    _createClass(Scene, [{
      ...
      value: function on(names, callback) {
        ...
        if (lodash.isFunction(callback)) {
          ...
          Log.log(1, "ERROR when calling '.on()': Supplied callback for '".concat(names, "' is not a valid function!"));
        }
      ...
      value: function addTo(controller) {
        if (this._controller !== controller) {
          ...
          Log.log(3, "added ".concat(NAMESPACE, " to controller"));
          this.update();
        }
      ...
      value: function remove() {
        if (this._controller) {
          ...
          this.trigger('remove');
          Log.log(3, "removed ".concat(NAMESPACE, " from controller"));
        }
      ...
      value: function destroy(reset) {
        ...
        this.off('*.*');
        Log.log(3, "destroyed ".concat(NAMESPACE, " (reset: ").concat(reset ? 'true' : 'false', ")"));
        return null;
      }
    }, {
      key: "update",
      ...
      value: function refresh() {
        ...
        this._updateTriggerElementPosition(); // update trigger element position
      ...
      value: function _validateOption() {
        ...
        for (var _len = arguments.length, check = new Array(_len), _key = 0; _key < _len; _key++) {
          ...
          if (_this7.validate[optionName]) {
            ...
            try {
              ...
              if (lodash.isArray(logMSG)) {
                logMSG[0] = "ERROR: ".concat(logMSG[0]);
                logMSG.unshift(1); // loglevel 1 for error msg

                Log.log.apply(_this7, logMSG);
              } else {
                Log.log(1, "ERROR: Problem executing validation callback for option '".concat(optionName, "':"), event.message);
              }
            } finally {// this.options[optionName] = value;
            }
          }
        });
      }
        ...
        if (!this[optionName]) {
          this[optionName] = function () {
            ...
            if (_this8._changeOption(optionName, arguments.length <= 0 ? undefined : arguments[0])) {
              // set
              _this8.trigger('change', {
                what: optionName,
                newVal: _this8.options[optionName]
              });

              if (SHIFTS.indexOf(optionName) > -1) {
                _this8.trigger('shift', {
                  reason: optionName
                });
              }
            }
        ...
        if (this._pin && this._controller && this._pinOptions.inFlow) {
          ...
          if (this._pinOptions.relSize.height) {
            if (during) {
              ...
              Util.css(this._pin, {
                height: Util.height(this._pinOptions.spacer) - (this._pinOptions.pushFollowers ? this.options.duration : 0)
              });
            } else {
              Util.css(this._pin, {
                height: '100%'
          ...
          if (this._pinOptions.pushFollowers) {
            css["padding".concat(vertical ? 'Top' : 'Left')] = this.options.duration * this._progress;
            css["padding".concat(vertical ? 'Bottom' : 'Right')] = this.options.duration * (1 - this._progress);
          }
      ...
      value: function _updateRelativePinSpacer() {
        if (this._controller && this._pin && this._state === SCENE_STATE_DURING && ( // element in pinned state? // is width or height relatively sized, but not in relation to body? then we need to recalc.
      ...
      value: function setPin(element, settings) {
        ...
        settings = lodash.merge({}, defaultSettings, settings); // validate element
        ...
        spacer.classList.add(settings.spacerClass); // set the pin Options
        ...
        spacer.appendChild(this._pin); // and set new css
        ...
        window.addEventListener('resize', this._updateRelativePinSpacer.bind(this), {
          passive: true
        ...
        Log.log(3, 'added pin'); // finally update the pin to init
      ...
      value: function removePin(reset) {
        if (this._pin) {
          ...
          window.removeEventListener('scroll', this._updatePinInContainer.bind(this), {
            passive: true
          });
          window.removeEventListener('resize', this._updatePinInContainer.bind(this), {
            passive: true
          });
          window.removeEventListener('resize', this._updateRelativePinSpacer.bind(this), {
            passive: true
          });
          ...
          Log.log(3, "removed pin (reset: ".concat(reset ? 'true' : 'false', ")"));
        }
      ...
      value: function setClassToggle(element, classes) {
        ...
        if (els.length === 0 || !lodash.isString(classes)) {
          Log.log(1, "ERROR calling method 'setClassToggle()': Invalid ".concat(els.length === 0 ? 'element' : 'classes', " supplied."));
          return this;
        }
      ...
      value: function setTween(TweenObject, duration, params) {
        ...
        try {
          ...
          if (window.TimelineMax) {
            newTween = new window.TimelineMax({
        ...
        this._tween = newTween; // some properties need to be transferred it to the wrapper, otherwise they would get lost.
      ...
      value: function removeTween(reset) {
        if (this._tween) {
          ...
          this._tween = null;
          Log.log(3, "removed tween (reset: ".concat(reset ? 'true' : 'false', ")"));
        }
      ...
      value: function addIndicators() {
        ...
        if (!this._indicator) {
          ...
          this.on('destroy.plugin_addIndicators', this.removeIndicators.bind(this)); // it the scene already has a controller we can start right away.
  ...
  function () {
    function Controller(options) {
      ...
      if (!this.options.container) {
        Log.log(1, "ERROR creating object ".concat(NAMESPACE$1, ": No valid scroll container supplied"));
        throw Error("".concat(NAMESPACE$1, " init failed"));
      }
      ...
      this._refreshTimeout = null; // normalize to window
      ...
      this._viewportSize = this._getViewportSize(); // set event handlers
      ...
      this._scheduleRefresh(); // indicators
      ...
      Log.log(3, "added new ".concat(NAMESPACE$1));
    }
    ...
    _createClass(Controller, [{
      ...
      value: function _updateScenes() {
        ...
        if (this._enabled && this._updateScenesOnNextCycle) {
          ...
          var scenesToUpdate = lodash.isArray(this._updateScenesOnNextCycle) ? this._updateScenesOnNextCycle : this._sceneObjects.slice(0); // reset scenes

          this._updateScenesOnNextCycle = false;
          var oldScrollPos = this._scrollPos; // update scroll pos now instead of on change, as it might have changed since scheduling (i.e. in-browser smooth scroll)
          ...
          scenesToUpdate.forEach(function (scene, index) {
            Log.log(3, "updating scene ".concat(index + 1, "/").concat(scenesToUpdate.length, " (").concat(_this._sceneObjects.length, " total)"));
            scene.update(true);
          });
      ...
      value: function addScene(newScene) {
        ...
        if (lodash.isArray(newScene)) {
          ...
          Log.log(3, "adding Scene (now ".concat(this._sceneObjects.length, " total)"));
        } // indicators
      ...
      value: function removeScene(scene) {
        ...
        if (lodash.isArray(scene)) {
          ...
          if (index > -1) {
            ...
            Log.log(3, "removing Scene (now ".concat(this._sceneObjects.length, " left)"));
            scene.remove();
          }
        }
      ...
      value: function updateScene(scene, immediately) {
        ...
        if (lodash.isArray(scene)) {
          ...
          scene.update(true); // if this._updateScenesOnNextCycle is true, all connected scenes are already scheduled for update
      ...
      value: function scrollTo(scrollTarget, additionalParameter) {
        if (lodash.isNumber(scrollTarget)) {
          ...
          var offset = this.options.vertical ? 'top' : 'left'; // container position is needed because element offset is returned in relation to document
      ...
      value: function destroy(resetScenes) {
        ...
        Log.log(3, "destroyed ".concat(NAMESPACE$1, " (reset: ").concat(resetScenes ? 'true' : 'false', ")")); // indicators
      ...
      value: function updateBoundsPositions(specificIndicator) {
        ...
        var groups = specificIndicator ? [lodash.merge({}, specificIndicator.triggerGroup, {
      ...
      value: function updateTriggerGroupPositions(specificGroup) {
        ...
        var containerOffset = this._isDocument ? {
          top: 0,
          left: 0
        ...
        var dimension = this._vertical ? 'width' : 'height';
        var transformAxis = this._vertical ? 'Y' : 'X'; // changing vars
        ...
        while (i--) {
          ...
          transform = pos > elSize ? "translate".concat(transformAxis, "(-100%)") : '';
          Util.css(el, {
          ...
          Util.css(el.firstChild.firstChild, {
            transform: transform
          });
        }
      ...
      value: function updateTriggerGroupLabel(group) {
        var text = "trigger".concat(group.members.length > 1 ? '' : " ".concat(group.members[0].options.name));
